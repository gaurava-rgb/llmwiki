...
If you used a [bulk action](<https://blog.readwise.io/reader/docs/faqs/highlights-tags-notes>) to delete an entire list of documents at once but then decided you actually wanted to be a bit more discerning than such wanton removal, it's no fun to restore those documents one at a time.
...
Yes! Reader supports a URL-based save format that lets you add any publicly available web article to your library directly. Prepend the URL with `https://wise.readwise.io/save?url=%60 and the visit the full link to save the article to your Reader inbox, just like pressing `A` in Reader and pasting the URL manually.

For example: `https://wise.readwise.io/save?url=https%3A%2F%2Fwww.example.com%2Farticle%60
...
> **Note:** PDFs in original view don't have this smooth stylus support yet, but if you [switch over to the text view](<https://blog.readwise.io/reader/docs/faqs/pdfs>) it will work great.
...
> **Tip:** To make the visual line between the columns a bit sharper (and to make it feel more like reading a real book), you might also like to turn on [justified text](<https://blog.readwise.io/reader/docs/faqs/eink>) while reading in this view.
...
However, the current logic isn’t able to detect duplicate _content_ , so saving the same document from slightly different URLs will result in the creation of a second version. This often happens when a URL contains tracking information or other additional parameters (e.g. `https://site-name.com/article-title vs `https://site-name.com/article-title`), and this is the primary reason that articles saved from a Feed source won’t be recognized as duplicates of the same articles saved from the original website.
...
In [long-form reading mode](<https://docs.readwise.io/reader/docs/faqs/appearance>) (the default view for EPUBs), the Listen button in the top right is replaced by the Appearance menu. To start TTS, open the **...** menu in the bottom right, then tap **Listen**.
...
![TTS pause button.](https://imgproxy.readwise.io/?url=http%3A%2F%2Fdocs.readwise.io%2Fimages%2Ftts-pause.png&hash=d9a9d65203a80c503f6a6f767757639d)
...
![](https://docs.readwise.io/images/send-to-kindle_config.png)
  ...
  3. Visit [Amazon's preferences page](<https://www.amazon.com/hz/mycd/myx>), and scroll to "Personal Document Settings."
...
![](https://docs.readwise.io/images/summarize-icon-web.png) ![](https://docs.readwise.io/images/summarize-button-mobile.jpeg)
